...
function PlayerCard({ player }) {
  ...
  const roleLabel = player.role || "RIFLER";
  const roleDescription = player.reason || "Роль рассчитана по доступной статистике FACEIT";
  ...
  return (
    ...
    >
      ...
      <div className="player-card__identity">
        ...
        {flag ? <span className="player-country" title={player.country}>{flag}</span> : null}
      </div>
      ...
      <div className="player-card__badges">
        ...
        {player.captain ? <span className="player-captain">CAPTAIN</span> : null}
      </div>

      <span className="player-role-note">роль по статистике FACEIT</span>
    </a>
  );
}
...
function RosterSkeleton() {
  return (
    <div className="roster-grid" aria-label="Загрузка состава команды">
      {Array.from({ length: 5 }, (_, index) => (
        <div className="player-card player-card--skeleton" key={index} aria-hidden="true">
    ...
    <section className="section roster-section" id="roster">
      ...
      {!loading && roster.length > 0 ? (
        <div className="roster-grid">
          {roster.map((player) => (
            <PlayerCard player={player} key={player.playerId || player.nickname} />
          ))}
        </div>
      ) : null}
